...
func PipeExample() error {
	...
	// this needs to be run in a separate go routine as it will block waiting for the reader
	// close at the end for cleanup
	go func() {
		// for now we'll write something basic, this could also be used to encode json base64, etc.
		defer w.Close()
		w.Write([]byte("test\n"))
	}()
